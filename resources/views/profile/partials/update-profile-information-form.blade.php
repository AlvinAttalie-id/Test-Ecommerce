<section>
    ...
    {{-- Avatar Preview --}}
    <div class="mt-4">
        <h3 class="text-sm font-semibold text-gray-700 mb-2">Avatar</h3>
        <img id="avatar-preview"
            src="{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('default-avatar.png') }}"
            alt="Avatar Preview" class="w-24 h-24 rounded-full object-cover border border-gray-300">
    </div>
    ...
    <form method="POST" action="{{ route('profile.update.avatar') }}" enctype="multipart/form-data" class="mt-4">
        ...
        @if (session('status') === 'avatar-updated')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                class="text-sm text-green-600 mt-2">{{ __('Avatar updated.') }}</p>
        @endif
    </form>
